...
$(document).on('click','.btnopenuser',function(){
	...
	if(c_user.hasClass('hidden')){
		...
		$.post('<?=base_url('customers/api_getsub') ?>',{parent:id,lv:lv}).done(function(d){
			if(d && d!='[]')
			{
				c_user.empty();
				d = JSON.parse(d);
				...
				$.each(d.data,function(i,u){
					 str += '<tr '+(d.lv==1?'class="success"':'class="info"')+'>'+
                              ...
                              '</td>'+
                              '<td style="width: 50px;"><a href="<?=base_url('customers/edit/'); ?>'+u.id+'" ><span class="glyphicon glyphicon-pencil"></span></a> '+
                                 '<a class="delete-confirm" href="<?=base_url('customers/delete/'); ?>'+u.id+'"><span class="glyphicon glyphicon-trash"></span></a>'+
                              '</td>'+
                           '</tr>'+
