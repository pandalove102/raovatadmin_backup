            ...
            <div class="ibox-content">
               <div class="form-horizontal">
                <form class="m-t" role="form" action="" method="post" id="form-users">
                    ...
                    <input type="hidden" name="id_user" id="id_user" value="<?=(isset($orders)) ? $orders->id : ''?>">
                     <div class="form-group"><label class="col-sm-2 control-label">Họ & Tên</label>
                        <div class="col-sm-9">
                           <input type="text" required="" placeholder="Full Name" class="form-control" name="fullname" value="<?=(isset($orders)) ? $orders->fullname : ''?>"">
                        </div>
                     </div>
                     ...
                     <div class="form-group"><label class="col-sm-2 control-label">Tên tài khoản</label>
                        <div class="col-sm-9">
                           <div class="relative">
                              <input type="text" <?=$this->uri == 'edit'?'disabled':'' ?> class="form-control area-input" rows="1" required="" placeholder="User Name"  name="username" id="username" data-error="Nhập tên tài khoản" data-error-1="Tài khoản đã tồn tại!" data-url="<?=base_url('orders/api_check_username')?>" value="<?=(isset($orders)) ? $orders->name : ''?>">
                           </div>
                     ...
                     <div class="form-group"><label class="col-sm-2 control-label">Email</label>
                        <div class="col-sm-9">
						   ...
						   data-url="<?=base_url('orders/api_check_email')?>" class="form-control" id="email" name="email" value="<?=(isset($orders)) ? $orders->email : ''?>">
                        </div>
                     </div>
                     <div class="hr-line-dashed"></div>
                     <div class="form-group"><label class="col-sm-2 control-label">Địa chỉ</label>
                        <div class="col-sm-9">
                           <input type="address" required="" placeholder="Địa chỉ" data-error="Nhập địa chỉ" class="form-control" id="address" name="address" value="<?=(isset($orders)) ? $orders->address : ''?>">
                        </div>
                     </div>
                     <div class="hr-line-dashed"></div>
                     <div class="form-group">
                        ...
                        <div class="col-sm-9">
                           <img src="<?=(isset($orders)) ? $orders->image : base_url('layout/images/no-image.png')?>" height="100px" width="100px">                  
                           <input type="hidden" name="image" value="<?=(isset($orders)) ? $orders->image : ''?>"  id="image" />
                        ...
                        <div class="col-sm-4">
                           <select class="form-control m-b" name="groupid">
                              <?php foreach ($this->data['list_group'] as $v) { ?>
                                 <option value="<?php echo $v->group_id ?>" <?=(isset($orders) && $orders->group_id==$v->group_id) ? 'selected="selected"' : '' ?>><?php echo $v->fullname ?></option>
                              <?php } ?>
                           </select>
                        </div>
                     </div>
                     ...
                     <div class="form-group"><label class="col-sm-2 control-label">Trạng thái</label>
                        <div class="col-sm-4">
                           <select class="form-control m-b" name="status">
                              <option value="1" <?=(isset($orders) && $orders->status==1) ? 'selected="selected"' : '' ?>>Enable</option>
                              <option value="2" <?=(isset($orders) && $orders->status==2) ? 'selected="selected"' : '' ?>>Disable</option>
                           </select>
                        </div>
                     </div>
                     <div class="hr-line-dashed"></div>
                     <div class="form-group">
                        <div class="col-sm-4 col-sm-offset-2">
                          <?=$this->randtoken('tokenoders'); ?>
                            <a class="btn btn-white" type="text" href="<?=base_url('orders');?>">Đóng</a>
